...
router.get('/pedidos', async (req, res, next) => {
  try {
    ...
    const fila = [...filaDelivery, ...filaTickets].sort((a, b) => (a.criado_em < b.criado_em ? -1 : 1));
...
router.post('/pedidos/:id/acao', async (req, res, next) => {
  try {
    ...
    const atualizado = await transicionarStatus(pedido.id, novoStatus);
    res.json({ pedido: atualizado });
  } catch (e) { next(e); }
});

/** Avança um TICKET de mesa/PDV (preparar / pronto). Some da fila quando fica pronto. */
router.post('/tickets/:id/acao', async (req, res, next) => {
  try {
    const c = req.cozinha!;
    const acao = textoLimpo(req.body.acao, 20);
    const novoStatus = ACOES_COZINHA[acao];
    ...
    const ticket = await db.prepare('SELECT id, status FROM cozinha_tickets WHERE id = ? AND loja_id = ?')
    ...
    if (novoStatus === 'pronto') {
      await db.prepare("UPDATE cozinha_tickets SET status = 'pronto', pronto_em = ? WHERE id = ?").run(agoraUTC(), ticket.id);
    } else {
      await db.prepare("UPDATE cozinha_tickets SET status = 'preparando' WHERE id = ?").run(ticket.id);
    }
    res.json({ ok: true });
  } catch (e) { next(e); }
});
