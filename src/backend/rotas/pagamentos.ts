...
export function getTokenMP(lojaId: number): string | null {
  const row = db.prepare('SELECT mercadopago_token FROM lojas WHERE id = ?').get(lojaId) as
...
export function pagamentoOnlineAtivo(lojaId: number): boolean {
  return !!getTokenMP(lojaId);
}
...
router.post('/webhook/mercadopago', async (req, res) => {
  try {
    ...
    const pedidoRow = db.prepare(
      'SELECT loja_id FROM pedidos WHERE pagamento_gateway_id = ?'
    ).get(String(pagamentoId)) as { loja_id: number } | undefined;
    const token = pedidoRow ? getTokenMP(pedidoRow.loja_id) : process.env.MERCADOPAGO_ACCESS_TOKEN;
    ...
    if (pedidoId) {
      ...
      const antes = db.prepare('SELECT pagamento_status FROM pedidos WHERE id = ?')
