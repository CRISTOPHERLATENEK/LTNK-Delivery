...
describe('criarOrdemPagamento', () => {
  it('manda valor decimal, charge_id e os dois headers', async () => {
    ...
    const r = await criarOrdemPagamento(CRED, { valorCentavos: 6990, chargeId: 'pedido-42' }, { buscar: f.buscar });
    ...
    expect(f.chamadas[0].headers.Authorization).toBe('Bearer TOKEN-SECRETO-DA-LOJA');
  ...
  it('sem serial, cai na lista geral (NRM) e não manda serial_pos', async () => {
    ...
    await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x' }, { buscar: f.buscar });
  ...
  it('com serial, vai direto para o aparelho (CRD_UNICO)', async () => {
    ...
    const f = fetchFalso({ corpo: { payment_identifier: 'p' } });
    await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x', serialPos: ' POS9 ' }, { buscar: f.buscar });
  ...
  it('installments só em crédito', async () => {
    ...
    await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x', tipo: 'CREDIT', parcelas: 3 }, { buscar: cred.buscar });
    expect(cred.chamadas[0].corpo.installments).toBe(3);

    const deb = fetchFalso({ corpo: { payment_identifier: 'p' } });
    await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x', tipo: 'DEBIT' }, { buscar: deb.buscar });
  ...
  it('crédito sem parcelas informadas vai como 1, não como ausente', async () => {
    const f = fetchFalso({ corpo: { payment_identifier: 'p' } });
    await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x', tipo: 'CREDIT' }, { buscar: f.buscar });
  ...
  it('resposta sem identificador é erro INDEFINIDO', async () => {
    ...
    await expect(criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x' }, { buscar: f.buscar }))
  ...
  it('timeout e queda de rede são INDEFINIDOS, não falha', async () => {
    ...
    const erro = await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x' }, { buscar: f.buscar })
      .catch(e => e);
  ...
  it('corpo ilegível não apaga o código HTTP', async () => {
    ...
    const erro = await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x' }, { buscar }).catch(e => e);
  ...
  it('valor inválido nem sai para a rede', async () => {
    const f = fetchFalso({ corpo: { payment_identifier: 'p' } });
    await expect(criarOrdemPagamento(CRED, { valorCentavos: 0, chargeId: 'x' }, { buscar: f.buscar })).rejects.toThrow();
    await expect(criarOrdemPagamento(CRED, { valorCentavos: 10.5, chargeId: 'x' }, { buscar: f.buscar })).rejects.toThrow();
  ...
  it('não tenta de novo sozinho', async () => {
    ...
    await criarOrdemPagamento(CRED, { valorCentavos: 100, chargeId: 'x' }, { buscar: f.buscar }).catch(() => {});
...
describe('consultarOrdem', () => {
  it('devolve a transação traduzida', async () => {
    ...
    const t = await consultarOrdem(CRED, 'pay-1', { buscar: f.buscar });
...
describe('cancelar, estornar e webhook', () => {
  it('cada um bate no seu endpoint', async () => {
    ...
    await cancelarOrdem(CRED, 'p1', { buscar: c.buscar });
    expect(c.chamadas[0].url).toContain('/order/status/cancelar');

    const e = fetchFalso({ corpo: {} });
    await estornarOrdem(CRED, 'p1', { buscar: e.buscar });
  ...
  it('o webhook registra a URL e o token que autentica o callback', async () => {
    ...
    await configurarWebhook(CRED, { url: 'https://x.com.br/hook?t=banco', tokenAutorizacao: 'seg' }, { buscar: f.buscar });
  ...
  it('erro em cancelamento propaga com a mensagem da API', async () => {
    ...
    await expect(cancelarOrdem(CRED, 'p1', { buscar: f.buscar })).rejects.toThrow('Já concluída');
  });
});
