...
async function criarUsuario(nome: string, email: string, senha: string, perfil: Perfil, telefone: string): Promise<number> {
...
async function criarContaCozinha(lojaId: number, nome: string, email: string, senha: string): Promise<void> {
  const existente = await db.prepare('SELECT id FROM cozinha_contas WHERE email = ?').get(email);
  if (existente) return;
  await db.prepare(
...
async function criarLoja(usuarioId: number, dados: DadosLoja): Promise<number> {
  const existente = await db.prepare('SELECT id FROM lojas WHERE usuario_id = ?').get(usuarioId) as { id: number } | undefined;
  if (existente) return existente.id;
  const info = await db.prepare(
...
async function criarProduto(lojaId: number, p: ProdutoSeed): Promise<void> {
  let produto = await db.prepare('SELECT id FROM produtos WHERE loja_id = ? AND nome = ?')
...
export async function seed(): Promise<void> {
  // ----- Contas de teste -----------------------------------------------------
  const adminId = await criarUsuario('Administrador', 'admin@demo.com', 'admin123', 'admin', '11999990000');
  // admin@demo.com é o SUPER ADMIN da plataforma de demonstração
  await db.prepare("UPDATE usuarios SET super_admin = 1 WHERE id = ?").run(adminId);
  // Admin operacional de exemplo (NÃO super)
  await criarUsuario('Ana Operadora', 'admin2@demo.com', 'admin123', 'admin', '11999991111');
  const clienteId = await criarUsuario('Carlos Cliente', 'cliente@demo.com', 'cliente123', 'cliente', '11988887777');
  const lojista1Id = await criarUsuario('Paula Pizzaiola', 'lojista@demo.com', 'lojista123', 'lojista', '11977776666');
  const lojista2Id = await criarUsuario('Bruno Burguer', 'lojista2@demo.com', 'lojista123', 'lojista', '11966665555');
  await criarUsuario('Edu Entregador', 'entregador@demo.com', 'entrega123', 'entregador', '11955554444');

  // ----- Pizzaria ------------------------------------------------------------
  const pizzariaId = await criarLoja(lojista1Id, {
  ...
  await criarContaCozinha(pizzariaId, 'Cozinha', 'cozinha@demo.com', 'cozinha123');
  ...
  for (const pizza of pizzasSalgadas) {
    await criarProduto(pizzariaId, { ...pizza, grupos: [TAMANHOS_PIZZA, BORDA_SALGADA, ADICIONAIS_PIZZA] });
  }
  await criarProduto(pizzariaId, {
  ...
  await criarProduto(pizzariaId, { nome: 'Refrigerante 2L', categoria: 'Bebidas', preco: 1400,
    descricao: 'Coca-Cola, Guaraná ou Soda.' });
  await criarProduto(pizzariaId, { nome: 'Suco de Laranja 1L', categoria: 'Bebidas', preco: 1600,
    descricao: 'Natural, feito na hora.' });
  await criarProduto(pizzariaId, { nome: 'Água com gás 500ml', categoria: 'Bebidas', preco: 600 });
  await criarProduto(pizzariaId, { nome: 'Petit Gâteau', categoria: 'Sobremesas', preco: 1890, promo: 1590,
    descricao: 'Bolinho de chocolate com recheio cremoso e sorvete de creme.' });

  // ----- Hamburgueria --------------------------------------------------------
  const burgerId = await criarLoja(lojista2Id, {
  ...
  for (const b of burgers) {
    await criarProduto(burgerId, { ...b, grupos: [EXTRAS_BURGER, COMBO_BURGER] });
  }
  await criarProduto(burgerId, { nome: 'Batata Frita Média', categoria: 'Acompanhamentos', preco: 1490,
  ...
  await criarProduto(burgerId, { nome: 'Onion Rings', categoria: 'Acompanhamentos', preco: 1690,
    descricao: 'Anéis de cebola empanados.' });
  await criarProduto(burgerId, { nome: 'Milk-shake de Ovomaltine', categoria: 'Bebidas', preco: 1990,
  ...
  for (const b of BANNERS_DEMO) {
    const ja = await db.prepare('SELECT id FROM banners WHERE titulo = ?').get(b.titulo);
    if (ja) continue;
    await db.prepare(
  ...
  for (const nomeAntigo of ['Pizza Quatro Queijos', 'Pizza de Chocolate']) {
    await db.prepare('UPDATE produtos SET excluido = 1, disponivel = 0 WHERE loja_id = ? AND nome = ?')
      .run(pizzariaId, nomeAntigo);
  }

  const temEndereco = await db.prepare('SELECT id FROM enderecos WHERE usuario_id = ?').get(clienteId);
  if (!temEndereco) {
    await db.prepare(
...
// Executa direto quando rodado como script (`npm run seed`); quando importado
// (ex.: SEED_ON_START em server.ts) só expõe a função `seed`.
if (require.main === module) {
  seed().catch(e => { console.error('❌ Falha no seed:', e); process.exit(1); });
}
